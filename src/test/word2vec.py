...
if len(sys.argv) < 2 or sys.argv[1] == '-h':
    print('params: \n\tdata, \n\tmodel, \n\tvector, \n\tmode(ori/nce/nce2)], \n\tnce_path{theta_w/theta_b}(if mode == "nce/nce2")'
          '\n\tinit_path{vector/theta_w/theta_b/vocab}(if mode == "nce2")')
    quit(0)

if sys.argv[4] == 'nce':
    _mode = 'n'
elif sys.argv[4] == 'nce2':
    _mode = 'n2'
else:
    _mode = 'o'
...
if _mode == 'n2':
    flags.DEFINE_string("init_vec_path", sys.argv[7], "Vector file to initial emb")
    flags.DEFINE_string("init_theta_w_path", sys.argv[8], "Theta_w file to initial")
    flags.DEFINE_string("init_theta_b_path", sys.argv[9], "Theta_b file to initial")
    flags.DEFINE_string("init_vocab_path", sys.argv[10], "Vocab file to initial")
...
class Options(object):
    ...
    def __init__(self):
        ...
        self.nce_path = ''
        if self.train_mode == 'n' or self.train_mode == 'n2':
            self.nce_path = '1'
            self.nce_theta_w_path = FLAGS.nce_theta_w_path
            self.nce_theta_b_path = FLAGS.nce_theta_b_path

        if self.train_mode == 'n2':
            self.init_vec_path = FLAGS.init_vec_path
            self.init_theta_w_path = FLAGS.init_theta_w_path
            self.init_theta_b_path = FLAGS.init_theta_b_path

            self.init_vocab_path = FLAGS.init_vocab_path
...
class Word2Vec(object):
    ...
    def __init__(self, options, session):
        ...
        if self._options.train_mode == 'n2':
            print('Building customed graph')
            self.build_graph2()
        else:
    ...
    def read_nce(self):
        ...
        if opt.nce_path != '':
            with open(opt.nce_theta_w_path, 'r') as f_theta_w,\
    ...
    def nce_sample(self, labels, batch_emb):
        ...
        print("rough_sampled_ids", rough_sampled_ids.get_shape, rough_sampled_ids.dtype)

        self._rough_sampled_ids = rough_sampled_ids
        ...
        self._real_nce_theta_w = tf.placeholder(dtype=tf.float32, shape=[opts.num_samples * 5, opts.emb_dim])
        self._real_nce_theta_b = tf.placeholder(dtype=tf.float32, shape=[opts.num_samples * 5])
        
        media_matrix = tf.matmul(batch_emb, self._real_nce_theta_w, transpose_b=True) + self._real_nce_theta_b
    ...
    def forward(self, examples, labels):
        ...
        example_emb = tf.nn.embedding_lookup(self._emb, examples)
        ...
        true_w = tf.nn.embedding_lookup(self._theta_w, labels)
        # Biases for labels: [batch_size, 1]
        true_b = tf.nn.embedding_lookup(self._theta_b, labels)

        # Weights for sampled ids: [num_sampled, emb_dim]
        sampled_w = tf.nn.embedding_lookup(self._theta_w, sampled_ids)
        # Biases for sampled ids: [num_sampled, 1]
        sampled_b = tf.nn.embedding_lookup(self._theta_b, sampled_ids)
    ...
    def load_vocab(self):
        opts = self._options
        self.vocab_words = []
        with open(opts.init_vocab_path, 'r') as f:
            for line in f:
                try:
                    word, count = line.strip().split(' ')
                    self.vocab_words.append(word)
                except ValueError:
                    pass
            opts.vocab_size = len(self.vocab_words)

    def load_vector(self):
        self._emb = None
        embs = []
        opts = self._options
        file_name = opts.init_vec_path
        print("Load vector from", file_name)
        ...
        self._emb = tf.Variable(tf.constant(embs, dtype=tf.float32))
        
        print("Lyy", self._emb.dtype)

    def load_theta_w(self):
        self._theta_w = None
        opts = self._options
        theta_ws = []
        file_name = opts.init_theta_w_path

        print("load theta_w from", file_name)
        with open(file_name, 'r') as f:
            for line in f:
                try:
                    theta_w = map(float, line.split(' '))
                    theta_ws.append(theta_w)         
                except ValueError as detail:
                    print("ValueError in loading theta_w:", detail)
                    pass
        self._theta_w = tf.Variable(tf.constant(theta_ws, dtype=tf.float32))


    def load_theta_b(self):
        self._theta_b = None
        opts = self._options
        theta_bs = []
        file_name = opts.init_theta_b_path
        print("load theta_b from", file_name)
        with open(file_name, 'r') as f:
            for line in f:
                try:
                    theta_b =float(line)
                    ...
                    print("ValueError in loading theta_b:", detail)
                    pass
        self._theta_b = tf.Variable(tf.constant(theta_bs, dtype=tf.float32))

    def build_graph2(self):
        ''' Customed graph initial vector by big train set'''
        ...
        self._word2id = dict()
        for i, w in enumerate(self.vocab_words):
            self._word2id[w] = i

        self.skipgram = skipgramreader.SkipgramReader(
                opts.batch_size,
                opts.window_size,
                opts.train_data,
                self._word2id
        ...
        opts.words_per_epoch = self.skipgram.word_per_epoch()
       
        self._epoch = 0
        self._words = 0

        self._examples = tf.placeholder(dtype=tf.int64, shape=[opts.batch_size])
        self._labels = tf.placeholder(dtype=tf.int64, shape=[opts.batch_size])
    ...
    def _lookup_nce(self, ids):
        _theta_w, _theta_b = [], []
        for idx in ids:
            _theta_w.append(self._nce_theta_w[idx])
            _theta_b.append(self._nce_theta_b[idx])
        return _theta_w, _theta_b

    def _train_thread_body(self):
        if self._options.train_mode == 'o':
            ...
            while True:
                # Get rough sampled ids
                if self._options.train_mode == 'n2':
                    EOF, batch = self.skipgram.get_next_batch()
                    if EOF:
                        if self._epoch == init_epoch:
                            self._epoch += 1
                        break
                    self._words += self._options.batch_size
                    self._batch += 1
                    np_batch = np.array(batch)
                    # print("np_0", np_batch[:, 0].dtype)
                    # print("np_1", np_batch[:, 1].dtype)
                    epoch = self._epoch
                    rough_sampled_ids = self._session.run(self._rough_sampled_ids,
                            feed_dict={self._labels:np_batch[:, 1]})
                    
                    self.real_nce_theta_w, self.real_nce_theta_b = map(np.array, self._lookup_nce(rough_sampled_ids))

                    _, loss = self._session.run([self._train, self._loss],
                            feed_dict={
                                self._examples: np_batch[:, 0],
                                self._labels: np_batch[:, 1],
                                self._real_nce_theta_w: self.real_nce_theta_w,
                                self._real_nce_theta_b: self.real_nce_theta_b
                                }
                            )
                    self.loss += loss
                else:
                    ...
                    real_nce_theta_w, real_nce_theta_b = map(np.array, self._lookup_nce(rough_sampled_ids))
                    _ = self._session.run(self._train,
                            feed_dict={
                                self._real_nce_theta_w: real_nce_theta_w,
                                self._real_nce_theta_b: real_nce_theta_b
                                })
                    epoch = self._session.run(self._epoch)
                    if epoch != init_epoch:
                        break
    ...
    def train(self):
        ...
        if opts.train_mode != 'n2':
            ...
            self.loss = 0
            self._batch = 0
            if self.skipgram.EOF:
                self.skipgram.initial()
        ...
        while True:
            ...
            if opts.train_mode != 'n2':
                (step, lr) = self._session.run( [self.global_step, self._lr])
                if opts.train_mode == 'o':
                    loss = self._session.run(self._loss)
                else:
                    loss = self.loss
                epoch, words = self._session.run([self._epoch, self._words])
            else:
                (step, lr) = self._session.run([self.global_step, self._lr])
                loss = self.loss / self._batch
                epoch, words = self._epoch, self._words
            now = time.time()
